...
3. **Lanza en paralelo** (single message, multiple Agent calls):
   ...
   Paquete de contexto (context isolation): base + branch + instrucción de leer `git diff <base>...HEAD` + lista de archivos + `BRIEF.md` + `DESIGN.md` + presupuesto + formato de salida. **Sin número de PR — no existe todavía.** Si el diff toca documentos normativos, sumá el **artefacto del paso 4 del DoD**: qué regla introduce el diff y qué encontró la relectura, incluido "nada". El reviewer no puede verificar lo que no recibió.
