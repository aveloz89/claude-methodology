...
**Bloquean el comando:** push directo a `main`, `gh pr merge --admin` (bypasea branch protections), `git push --force`, `git reset --hard`, y `gh pr merge` fuera de la forma `gh pr merge <N> [flags conocidos]` sola en el comando y en una línea — sin número de PR explícito, con threads de review sin resolver, con reviews/checks pendientes (fail-closed si no puede verificar), o con cualquier prefijo (`cd`, variables de entorno, `&&`/`;`/`|`) antes del merge. Para mergear un PR de otro repo, usa `--repo`/`-R` explícito, nunca `cd`. Si uno te bloquea, la solución nunca es esquivarlo.
