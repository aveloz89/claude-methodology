...
**Bloquean el comando:** push directo a `main`, `gh pr merge --admin` (bypasea branch protections), `git push --force`, `git reset --hard`, y `gh pr merge` sin número de PR explícito, con threads de review sin resolver, o con reviews/checks pendientes (fail-closed si no puede verificar). Para mergear un PR de otro repo, usa `--repo` explícito — cualquier otro prefijo o forma no estándar antes del merge bloquea. Si uno te bloquea, la solución nunca es esquivarlo.
