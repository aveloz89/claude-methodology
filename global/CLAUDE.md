...
**Bloquean el comando:** push directo a `main`, `gh pr merge --admin` (bypasea branch protections), `git push --force`, `git reset --hard`, y —cuando reconoce la invocación (detalle de qué formas reconoce en `hooks/pre-merge-check.sh`)— `gh pr merge` fuera de la forma `gh pr merge <N> [flags conocidos]` sola en el comando y en una línea: sin número de PR explícito, con threads de review sin resolver, con reviews/checks pendientes (fail-closed si no puede verificar). Para mergear un PR de otro repo, usa `--repo`/`-R` explícito, nunca `cd`. Si uno te bloquea, la solución nunca es esquivarlo.

**Corren en background:** tests antes de cada commit (se omiten cuando lo único con cambios locales en el árbol es `.planning/`, y el comando no redirige git a otro árbol), checkpoint de review al crear un PR (verifica que el review dual pre-push ocurrió; solo instruye lanzarlo para PRs fuera del flujo), contexto de sesión al arrancar, aviso de contexto agotándose (35% / 25%), detección de servicios Docker que necesitan restart, `latent-bugs-sweep` antes de un `gh pr create --base main`, snapshot de `.planning/` antes de compactar, log de invocaciones de subagentes, y verificación de STATE desactualizado al cerrar sesión.
