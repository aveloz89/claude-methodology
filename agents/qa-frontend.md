...
- **`~/.claude/rules/implementation-principles.md`** — YAGNI, cambios quirúrgicos, no stubs/TODOs, no error handling defensivo, verificar antes de afirmar (§5: si el diff arregla un bug, borrar la línea del fix debe poner algún test en rojo — si sigue verde, es bloqueante). La regla de "validación solo en boundaries" sale de ahí.
