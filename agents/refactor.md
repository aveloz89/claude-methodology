...
**Entregas:**
- Si `last_batch=true` → verificación final + commits locales + reporte "listo para review dual + push + PR" (los hace el orchestrator, como con los demás devs)
...
- Si **no lográs construir** un input que ejercite la rama → se reporta como *no verificado* y **no se borra**. Lo mismo si el input **cuelga o no termina**: no es un rojo ni un verde, es un experimento sin resultado. Acotalo en tiempo (`rules/bash.md`, sección Procesos) y si aun así no concluye, tratalo como si no lo hubieras podido construir. "No pude probar que hace falta" no es "probé que no hace falta".
