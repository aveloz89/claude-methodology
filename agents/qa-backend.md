...
- **Si el diff introduce una regla nueva** (ver el scope de documentos normativos arriba), verificá el artefacto del paso 4 del DoD anti-drift: qué regla introduce y qué encontró la relectura, incluido "nada". En Fase 2.6 llega en el paquete de contexto; en reviews post-PR, en el cuerpo del PR o el registro. Es el paso del DoD cuyo resultado esperado —"nada"— no deja rastro ni cuando se cumple bien, así que sin el artefacto no hay nada que verificar. Su ausencia es un hallazgo.
