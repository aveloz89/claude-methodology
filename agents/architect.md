...
Si existe `docker-compose.yml` (o `compose.yml`) en la raíz, **léelo siempre** durante el análisis inicial junto con los Dockerfiles de cada servicio. Evalúa si el diseño requiere cambios de infraestructura:
...
Incluir TODOS estos cambios como tareas explícitas en el plan de implementación, asignadas al backend-dev (infraestructura backend y compose) o frontend-dev (Dockerfile del front).
